...
VarBindingString *QueryProcessor::searchJoin(vector<TripleString> &patterns,
                                             set<string> &vars) {
  try {
    ...
    for (vector<TripleString>::iterator itPat = patterns.begin();
      ...
      if (IS_VARIABLE(triple.getSubject())) {
        ...
        varRole.insert(
            pair<string, TripleComponentRole>(triple.getSubject(), SUBJECT));
      }

      if (IS_VARIABLE(triple.getPredicate())) {
        neededVars.insert(triple.getPredicate());
        varRole.insert(pair<string, TripleComponentRole>(triple.getPredicate(),
                                                         PREDICATE));
      }
      ...
      if (IS_VARIABLE(triple.getObject())) {
        ...
        varRole.insert(
            pair<string, TripleComponentRole>(triple.getObject(), OBJECT));
      }
    } // Gather all appearing vars.
    ...
    for (vector<TripleString>::iterator itPat = patterns.begin();
      ...
      vector<unsigned char>
          vars; // each position means: 1 Subject, 2 Predicate, 3 Object.
      vector<string> varnames; // If not bound, return null.

      // Fill array indicating which vars we are going to extract from the
      // triple pattern.
      ...
      VarBindingInterface *bind = new TriplePatternBinding(
          hdt->getTriples(), triplePatID, vars, varnames);
      std::cout << "Pattern: " << *itPat
                << " Results: " << bind->estimatedNumResults() << std::endl;
      ...
      for (list<VarBindingInterface *>::iterator it = items.begin();
        ...
        if (commonVars.size() > 0) {
          ...
          if (left->estimatedNumResults() > 200000 &&
            ...
            // TODO: Heuristic to use presort+mergejoin when the number of left
            // results is big.
    ...
    return new BasicVarBindingString(varRole, new VarFilterBinding(root, vars),
                                     hdt->getDictionary());
  } catch (char *e) {
